---
layout: default
title: '001. # Políticas de Ramas y Commits para Poli-Tutorias > Redactado por…'
parent: Redacción TIC DevOps
nav_order: 1
permalink: /07-tesis-devops/001/
---
...
**Sección:** Redacción TIC DevOps &middot; **Longitud:** 23,137 caracteres &middot; **Posición en la conversación:** 1 de 24

---

```
# Políticas de Ramas y Commits para Poli-Tutorias
...
Dada la presencia del rol de Agile Testing en nuestro equipo, he implementado un modelo de **Git Flow modificado con tres ramas permanentes** que permite una clara separación de ambientes y facilita el proceso de validación antes del despliegue a producción.
...
- **Ambiente Azure:** AKS Development
...
  - Genera despliegue automático a ambiente de desarrollo via pipeline.
...
  - Genera despliegue automático a ambiente de QA via pipeline.
...
HU00-T07 ci(docker): optimizar imagen de producción




Reduce tamaño de imagen de 850MB a 320MB usando:
...
HU00-T08 chore(docker): agregar servicio de Redis para caché
...
HU02-T12 fix(backend): corregir validación de email en registro
...
HU03-T15 refactor(backend): optimizar consulta de tutores disponibles
...
HU01-T18 perf(backend): mejorar tiempo de respuesta de endpoint de búsqueda
...
| `.gitignore` | `chore` | `HU00-T01 chore(repo): actualizar .gitignore` |

| `.env`, `.env.example` | `chore` o `ci` | `HU00-T02 chore(config): agregar variables de entorno` |

| `README.md` | `docs` | `HU00-T03 docs(readme): actualizar instrucciones` |

| `package.json` | `chore` | `HU00-T04 chore(deps): actualizar dependencias` |

| `Dockerfile` | `ci` | `HU00-T05 ci(docker): optimizar imagen` |

| `docker-compose.yml` | `chore` | `HU00-T06 chore(docker): agregar servicio Redis` |

| `.eslintrc` | `style` | `HU00-T07 style(config): configurar ESLint` |

| `tsconfig.json` | `chore` | `HU00-T08 chore(config): habilitar strict mode` |

| `azure-pipelines.yml` | `ci` | `HU00-T09 ci(pipeline): agregar stage de testing` |

| `LICENSE` | `docs` | `HU00-T10 docs(legal): agregar licencia MIT` |
...
   git checkout -b feature/HU01-busqueda-tutores
...
   git commit -m "HU01-T05 feat(frontend): agregar barra de búsqueda de tutores"
...
   git commit -m "HU01-T05 style(frontend): ajustar diseño de barra de búsqueda"

   git commit -m "HU01-T05 test(frontend): agregar pruebas para búsqueda"
...
   git push origin feature/HU01-busqueda-tutores
...
4. **Despliegue automático a ambiente Development**
...
   - El pipeline automático despliega a ambiente QA
...
   git checkout -b hotfix/HU05-caida-autenticacion
...
   git commit -m "HU05-T30 fix(backend): corregir timeout en autenticación"
...
   git merge hotfix/HU05-caida-autenticacion
...
Cada rama permanente tiene su pipeline asociado:




- **Pipeline de `develop`:** Build, tests, análisis de código, deploy a AKS Development

- **Pipeline de `qa`:** Build, tests exhaustivos, deploy a AKS QA/Staging

- **Pipeline de `main`:** Build, tests, validaciones finales, deploy a AKS Production
...
- **Azure DevOps Branching Strategies:** [Microsoft Docs - Git branching guidance](https://learn.microsoft.com/en-us/azure/devops/repos/git/git-branching-guidance)
...
- **Azure DevOps Work Item Linking:** [Microsoft Docs](https://learn.microsoft.com/en-us/azure/devops/boards/backlogs/add-link)
...
Modifica esto mencionando que es para static web apps el front y back por el momento con blob storage de azure para las imagnes
```
...
[Volver a Redacción TIC DevOps](/07-tesis-devops/) &middot; [Prompt 002 →](/07-tesis-devops/002/)
{% endraw %}
